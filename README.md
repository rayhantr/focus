...
- **Taskbar info**: a clock-style cell rendered right next to the Windows time/date — next prayer + its time on line one, a live countdown on line two (amber when the waqt is near, red while locked). Technically a topmost top-level tool window (`ui/taskbar`) pinned over the taskbar left of `TrayNotifyWnd` in physical pixels (`winchrome.ts` `attachTaskbarCell`) and re-asserted every minute — the Win11 taskbar re-asserts its own topmost, so the placement pass asserts the cell right back. (Cross-process `GWLP_HWNDPARENT` ownership silently fails, so ownership is *not* the mechanism — see the `winchrome.ts` header.) The tray icon remains for the menu + a live tooltip.
